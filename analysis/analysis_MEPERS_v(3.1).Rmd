---
title: "Maine Public Employee Retirement System (MainePERS): Modeling approach and simulation results"
output:
  html_notebook:
    toc: yes
    number_sections: yes
...
To examine the impact of the MainePERS risk-sharing features on plan costs, funded status, and member benefit, we run the simulation under the following three policy scenarios.

1.  **Shared-ADC and variable COLA** (MainePERS policy)
...
We need to better understand how the adjustable COLA is reflected in actuarial valuation calculations of liability in MainePERS. We are not sure what assumption they use about future COLAs when calculating the COLA reduction needed to bring the contribution rates under the caps. There are at least two possibilities:
...
tbl_replacment <- 
  df_benefit_y30$benefit_qtile %>% 
	...
	tab_source_note(
		source_note = "Note: benefits at age 75 are adjusted for inflation; assumed inflation = 2.75% (MainePERS assumption) "
	)
