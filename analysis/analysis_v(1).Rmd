---
title: "Simulation results of MEPERS"
output: 
  html_notebook: 
    number_sections: yes
    toc: yes
...
names(runs_select) <- names(runs_select)
...
results_all %>%
	filter(sim_name == "Dev_noRS", sim == 5) %>%
	select(sim_name, sim, year, ERC, EEC_PR, NC_PR)
...
df_CAGR <- 
df %>% 
	filter(sim_name == "Dev", sim >= 1, year <= 2020 + span ) %>% 
	group_by(sim) %>% 
...
df_ERCMaxChg <- 		
  ...
  filter(sim >= 1, year <= span) %>% 
	...
	summarise(sim_namel = unique(sim_name),
