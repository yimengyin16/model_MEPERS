...
liab_active %<>% 
  ...
  mutate(
    ...
    ## Bx with benefit factor reduction for future services
    # N/A for MEPERS
    ...
    qxm_defrRet = ifelse(age < 40, qxm.pre, qxm.post),  #  MEPERS
    
    
    pxm_active  = 1 - qxm.pre,   # TODO: change variable name for qxmd.post; MEPERS: qxm.pre = 0 for age >= 70
...
liab_active %<>% 
  mutate( gx.disbRet  = case_when(
                          grp %in% c("regularAll") ~ as.numeric(yos >= 5),
          ...
          # yos_proj_disbRet = ifelse(age <= 60, 60 - ea, yos), 
  				...
  				Bx.disbRet  = case_when(
  				    ...
  				    grp %in% c("regularAll" ) ~ gx.disbRet * 0.59 * fas,
  				    TRUE ~ 0
  				  ),
...
liab_active %<>%
	mutate( gx.death  = 1,
					...
					Bx.death = 1 * sx * gx.death,   # pmax(0, gx.death * sx * 3 - 50000),
