...
#' What this file does
#'   - produce the following model inputs for tier "regular2t"
#'     - decrement table
#'     - salary scale
...
#'  
#'  - Benefit rules
#'      - Use benefit rules of Special plan 2C
#'      
#'       
#'      
...
#'  
#'  - Benefit formula
#'      - benfactor: 50% of FAS plus 2% for each year of YOS greater than 25
#'  
#'  
#'  
#   - Normal retirement age
#       - N/A, no early retirement reduction.
#'  
#'  - Eligibility
#'      - YOS >= 25 or
#'      
...
# Notes on retirement rates
#  - AV ret rates are based on yos,
#  - we also assume the max retirement age is 70 (rate applied at age 69)
...
# Tier specific parameters
tier_name <- "special"
age_vben  <- 60 # assumed age of starting receiving deferred retirement benefits
v.year    <- 5
fasyears  <- 3  
bfactor   <- 0.02
...
# groups included
grp_include <- df_qxr_special_imputed$grp %>% unique
grp_include <- grp_include[str_detect(grp_include , "special")]

# weight for each group
wgts <- tibble(grp = grp_include, wgt = 0)

wgts[wgts$grp == "special","wgt"] <-  1
...
df_qxm.pre_tier <-  
  df_qxm.pre_imputed %>% 
  mutate(qxm.pre = 0.1 * qxm.pre.female + 0.9 * qxm.pre.male,
         grp = tier_name
         ) %>% 
...
df_qxm.post_tier <-  
  df_qxm.post_imputed %>% 
  mutate(qxm.post         = 0.1 * qxm.post.female  + 0.9 * qxm.post.male,
         grp = tier_name
  ) %>% 
...
df_qxmd.post_tier <-  
  df_qxmd_imputed %>% 
  mutate(qxmd.post = 0.1 * qxmd.female  + 0.9 * qxmd.male,
         grp  = tier_name
  ) %>% 
...
decrements_tier <- 
  ...
  left_join(df_qxr_tier,            by = c("grp", "yos")) %>%         # service retirement
...
decrements_tier  %<>% 
  ...
  mutate(
    ...
    elig_servRet_full = ifelse( yos >= 25, 1, 0) %>% cumsum,
    
    # Eligibility for early retirement benefit
    elig_servRet_early = ifelse( yos >= 25, 1, 0) %>% cumsum,
  ...
  #   2. Coerce termination rates to 0 when eligible for early retirement or full retirement, or age >= age_vben or
  #   3. For Special plans in MEPERS: coerce retirement age to 1 at age 70
  
  mutate(
    qxr        = ifelse(elig_servRet_early | elig_servRet_full, qxr, 0),
    qxr        = ifelse(age >= 70, 1, qxr), 
